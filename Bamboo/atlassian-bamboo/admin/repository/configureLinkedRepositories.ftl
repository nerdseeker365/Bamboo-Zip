...
<div id="panel-editor-setup" class="repository-config[#if !repositoryDefinitions?has_content] no-items[/#if]">
    ...
    <div id="panel-editor-list">
        ...
        [#list repositoryDefinitions as repoDef]
            [#-- TODO: https://extranet.atlassian.com/jira/browse/BDEV-12678, property should be read from the repoDef --]
            [#assign isCiEnabled = false /]
            [@repositoryListItem id=repoDef.id name=repoDef.name!?html isCiEnabled=isCiEnabled /]
        [/#list]
        </ul>
...
[@repositoryListItem id="{id}" name="{name}" isCiEnabled=false /]
</script>
...
[#macro repositoryListItem id name isCiEnabled=false]
<li class="item" data-item-id="${id}" id="item-${id}">
    ${soy.render("aui.lozenges.lozenge", {
        "extraClasses": "repository-item-lozenge ${isCiEnabled?then('', 'hidden')}",
        "text": "CI enabled",
        "type": "success",
        "isSubtle": true
    })}
