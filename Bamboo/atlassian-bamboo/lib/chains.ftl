                ...
                [#local stageStatus]
                    [@ui.standardMenu triggerText=menuTriggerText id="actions-menu-${stage.id}" icon="configure" useIconFont=true subtle=true compact=true iconOnly=true]
                        ...
                        [#if fn.hasPlanPermission("ADMINISTRATION", chain)]
                            [@ww.url var='confirmDeleteStageLink' action='confirmDeleteStage' namespace='/chain/admin' buildKey=key stageId=stage.id returnUrl=currentUrl /]
                                        ...
                                        [#if fn.hasPlanPermission("BUILD", chain)]
                                            <li>
                                                [#if job.suspendedFromBuilding]
                                                    [@ww.url var="resumeBuildLink" namespace="/build/admin" action="resumeBuild" buildKey=job.key returnUrl=currentUrl/]
                                                    [@ui.displayLink href=resumeBuildLink titleKey="global.buttons.enable" mutative=true/]
                                                [#else]
                                                    [@ww.url var="suspendBuildLink" namespace="/build/admin" action="suspendBuild" buildKey=job.key returnUrl=currentUrl/]
                                                    [@ui.displayLink href=suspendBuildLink titleKey="global.buttons.disable" mutative=true/]
                                                [/#if]
                                            </li>
                                        [/#if]
                                        [#if fn.hasPlanPermission("ADMINISTRATION", chain)]
                                            [@ww.url var="deleteJobLink" action="deleteChain" namespace="/chain/admin" buildKey=job.key returnUrl=currentUrl /]
