...
[#macro displayTabs selectedTab location innerLocation="" linkCssClass="" analyticsEvent=""]
    ...
    [#list action.getWebSectionsForLocation(location) as section ]
        ...
        [#list actionItems as item]
            ...
            [@displaySingleTab item=item selectedTab=selectedTab id=id url=url linkCssClass=linkCssClass sectionKey=section.key analyticsEvent=analyticsEvent/]
        ...
        [#if innerLocation?has_content]
            [#list action.getWebSectionsForLocation(innerLocation) as innerSection ]
                ...
                [#list actions.keySet() as item]
                    ...
                    [@displaySingleTab item=item selectedTab=selectedTab id=id url=url linkCssClass=linkCssClass analyticsEvent=analyticsEvent/]
                    [#if selectedTab == item.name]
...
[#macro displaySingleTab item selectedTab id url linkCssClass sectionKey="" analyticsEvent=""]
    <li class="menu-item[#if selectedTab == item.name] active-tab[/#if]" [#if sectionKey?has_content]data-section="${sectionKey}"[/#if]>
        <a [#t]
...
[#macro displayPlanDeleteLink plan labelKey returnUrl]
    [#if fn.hasPlanPermission('ADMINISTRATION', plan)]
        [@ww.url var='deletePlanUrl' action="deleteChain" namespace="/chain/admin" buildKey="${plan.key}" returnUrl=returnUrl/]
...
[#macro displayPlanYamlLink plan labelKey]
    [#if featureManager.isExportToYamlUiEnabled()
        && fn.hasPlanPermission('ADMINISTRATION', plan)]
...
[#macro displayPlanSpecsLink plan labelKey]
    [#if featureManager.isExportToYamlEnabled()
    && fn.hasPlanPermission('ADMINISTRATION', plan)]
