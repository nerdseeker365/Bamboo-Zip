define([
    ...
    test('schedules next update on init when the page is visible', function() {
        ...
        equal(new Navigator().scheduleNextUpdate.callCount, 1, 'scheduleNextUpdate method should be called on init');
    });

    test('doesn\'t schedule next update on init when the page is not visible', function() {
        var Navigator = PlanStatusHistory.extend({
            scheduleNextUpdate: sinon.spy()
        });
        ...
        equal(new Navigator().scheduleNextUpdate.callCount, 0, 'scheduleNextUpdate method should not be called on init');
    });
